...
**Verdict: INVALID (Methodological Asymmetry Discovered)** — Trace runner violated matched-lifecycle contract:

1. Arm order not alternated: all 3 cycles ran shared→isolated, no AB/BA alternation.
2. Warmup asymmetry: shared arm used warmup_first=True, isolated arm used False.
3. Namespace asymmetry: shared reused "audit-shared" across cycles; isolated used fresh per-cycle/per-instance namespaces with different residency history.
4. Server lifecycle not independent: server started once for all 3 cycles, not restarted per arm.
5. Aggregate median TTFT nearly identical (149.3ms vs 150.3ms); mean difference (213.7ms) comes almost entirely from Q0 (329.5ms vs 924.6ms). Mixed-class mean cannot be interpreted as unified prefill saving.
6. DMA cost reported as 45.5ms full-shared-mean, not per-request paired to corresponding hit.

**Artifact retained as negative example of methodological asymmetry.** Runner fix (AB/BA alternation, symmetric warmup, independent server lifecycles, per-request DMA binding, per-query-class paired reporting) committed in `run_trace_audit.py` but NOT re-run on NPU pending host-only review.
