...
PagedAttention establishes block-granular KV-cache management inside an LLM
serving engine~\cite{kwon2023pagedattention}, while Mooncake makes KV state a
disaggregated serving resource and reports the storage--computation tradeoff of
moving it across a KV-centric data path~\cite{qin2025mooncake}.  These systems
motivate, but do not themselves establish, PegaFlow's proposed receipt-bound
break-even predictor.  Prefix caches establish reuse within an engine; generic
transfer engines optimize bytes, registration, and fabric movement; queueing
and SLO controllers choose request-level service actions.
