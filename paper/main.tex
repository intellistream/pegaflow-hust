...
\title{PegaFlow: Tail-Predictable Host-Memory Pooling for Disaggregated LLM Serving}
...
and transfer budgeting at the cache/runtime boundary. Multi-path selection,
connection recovery, and generation-safe object identity remain required
...
The primary gate is a preregistered reduction in RDMA and request p99 relative
to the strongest deployable baseline at unchanged total CPU memory and
non-inferior cache hit. Baseline repeats must freeze the numerical margin before
treatment data are inspected. A 50\% increase in effective cache residence is
